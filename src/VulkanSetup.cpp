...
void VulkanSetup::createImage(uint32_t width, uint32_t height, VkFormat format,
...
void VulkanSetup::transitionImageLayout(VkImage image, VkFormat format,
                                        VkImageLayout oldLayout,
                                        VkImageLayout newLayout,
                                        VkCommandPool* commandPoolPtr) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(commandPoolPtr);
    ...
    endSingleTimeCommands(commandBuffer, commandPoolPtr);
}

void VulkanSetup::copyBufferToImage(VkBuffer buffer, VkImage image,
...
void VulkanSetup::populateDebugMessengerCreateInfo(
...
VulkanSetup::querySwapChainSupport(VkPhysicalDevice device) {
...
void VulkanSetup::createSwapChain() {
    ...
    createInfo.preTransform = swapChainSupport.capabilities
                                  .currentTransform; // Don't transform images
    createInfo.compositeAlpha =
        VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR; // Ignore alpha channel
...
void VulkanSetup::recreateSwapChain(VkCommandPool* commandPoolPtr) {
    ...
    // this doesn't trigger if the window is minimized
    // on Fedora 39 KDE Plasma (5.27.9) Wayland.
    // Vulkan 5.27.9
    // Mesa 23.2.1
    ...
    createSwapChain();
    createImageViews();
    createDepthResources(commandPoolPtr);
    createFramebuffers();
}
...
void VulkanSetup::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        framebufferInfo.renderPass = *renderPassPtr;
...
VkExtent2D
VulkanSetup::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
void VulkanSetup::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void VulkanSetup::createIndexBuffer(VkCommandPool* commandPoolPtr,
    ...
    copyBuffer(stagingBuffer, indexBuffer, bufferSize, commandPoolPtr);
...
void VulkanSetup::createDescriptorPool() {
    const int numDescriptorSets = MAX_FRAMES_IN_FLIGHT * textures.size() + 1;
...
void VulkanSetup::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                             VkDeviceSize size, VkCommandPool* commandPoolPtr) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(commandPoolPtr);
...
VkCommandBuffer
VulkanSetup::beginSingleTimeCommands(VkCommandPool* commandPoolPtr) {
...
void VulkanSetup::endSingleTimeCommands(VkCommandBuffer commandBuffer,
    ...
    vkFreeCommandBuffers(device, *commandPoolPtr, 1, &commandBuffer);
}

uint32_t VulkanSetup::findMemoryType(uint32_t typeFilter,
                                     VkMemoryPropertyFlags properties) {
...
void VulkanSetup::createDepthResources(VkCommandPool* commandPoolPtr) {
...
bool VulkanSetup::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT ||
           format == VK_FORMAT_D24_UNORM_S8_UINT;
}

VkFormat
VulkanSetup::findSupportedFormat(const std::vector<VkFormat>& candidates,
